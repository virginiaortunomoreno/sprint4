...
function getMoviesFromDirector(array, director) {
  ...
  let result = array.filter(mov => mov.director === director).map(filteredmov => filteredmov.title);
...
function moviesAverageOfDirector(array, director) {
  ...
  result = (result/scoreMoviesDirector.length).toFixed(2);
...
function orderAlphabetically(array) {
  ...
  let result = orderAlpha.slice(0,20).map(newOrder => `${newOrder.title}`);
